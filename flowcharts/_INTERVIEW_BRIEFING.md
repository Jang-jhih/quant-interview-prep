...
> **用途**：針對 `flowcharts/` 的 **9 份**架構流程圖與回測報告，準備面試時口語說明用的稿本。
> 每個主題獨立一章，每章包含：
>
> 1. **30 秒電梯簡報** — 一上來怎麼開口
> 2. **架構說明要點** — 講解時的邏輯順序
...
>
> 配合閱讀：
> - [`_FACTS.md`](./_FACTS.md) — **數字背這份**，不要背各流程圖裡的數字（避免版本漂移）
> - [`SOURCE_MANIFEST.md`](./SOURCE_MANIFEST.md) — 每份流程圖參考的程式碼路徑

### 章節索引

| 章     | 主題                       | 判定 / 性質             |
| ----- | ------------------------ | ------------------- |
| 五     | 事件型市場風險評估（總架構）           | 方法論主體               |
| **九** | **槓桿守門 Overlay（Pack D）** | **CONDITIONAL**     |
| **八** | **恐慌抄底訊號（Pack B）**       | ✅ **IS+OOS 雙 KEEP** |
| 六     | FinGPT 恐慌指數（Pack C）      | 降格為 overlay         |
| 一     | 股票池篩選研究（GA 選股）           | 研究框架，未實盤            |
| 二     | LLM 驅動因子自動演化             | 研究框架                |
| 三     | 多源金融資料倉儲                 | 生產運行                |
| 四     | 統一 AI 服務編排層              | 生產運行                |
| 七     | 產業輪動風險（Pack C）           | v1 LOCK             |
| **十** | **策略回測報告（17 份）**         | 研究層候選篩選             |

> **面試策略**：如果只有 10 分鐘，講 **八 + 九**——一個「真的過了」，一個「紀律最嚴」。
> 其他章是背景與方法論鋪陳。
...
> 真正做到扣成本、含融資成本、還開了 hold-out 的是 Pack D 的槓桿守門議題（見第九章）。
...
5. **最後講兩次 pivot**——這是最大亮點，但**必須講成一條因果鏈，不是兩個獨立事件**：
   ...
   | ⑤ | 用修正後的全歷史重跑子議題 → **`fingpt_panic_rebound` 取得 IS+OOS 雙 KEEP**（第八章） |
...
## 八、恐慌抄底訊號（fingpt_panic_rebound · Pack B）★ 唯一雙 KEEP
...
1. **先講它是誰的續集**——用的是第六章那個 FinGPT 恐慌指數。
   因為第六章修正了「只有 2024 之後資料」的錯誤前提，這題才拿得到全歷史 4141 天
2. **再講三條假設，兩條死掉**：
...
**Q：同一個 panic 指標，第六章說不能宣稱方向，這裡又說能預測反彈？**
> 因為**問的不是同一件事**。第六章（Pack C）問「這個分數能不能描述當下的波動狀態」——那是同期相關；
...
## 九、槓桿守門 Overlay（leverage_guard_overlay · Pack D）★ 紀律最嚴
...
> 唯一雙 KEEP 的那個訊號（第八章），我還是只敢說做到 paper trading。
...
- **被問「哪個策略最好」**：不要選一個吹。答「這批的用途是挑方向，
  真正被我拿去做完整驗證的是恐慌抄底那個訊號」——把話題導回第八章
...
- [ ] **數字只背 [`_FACTS.md`](./_FACTS.md)**，不要背流程圖裡的（避免版本漂移）
...
- [ ] 10 分鐘版本的取捨演練：**只講第八章（雙 KEEP）+ 第九章（事前登記）**
